...
# Load the WW woodworking helper library
require_relative 'woodworking'
puts "WW Woodworking library loaded"
...
module SU_MCP
  class Server
    ...
    def perform_difference(target, tool, result_group)
      ...
      # Subtract the tool from the result (use Group#subtract, not Entities#subtract)
      new_result = result_group.subtract(temp_tool_group)

      # Clean up temporary copies
      target_copy.erase!
      tool_copy.erase!
      # Note: subtract erases both input groups and returns a new group
    end
    ...
    def create_mortise(board, width, height, depth, face_direction, bounds, offset_x, offset_y, offset_z)
      ...
      # Subtract the mortise from the board (use Group#subtract, not Entities#subtract)
      result = board.subtract(mortise_group)
      # subtract erases both input groups and returns a new group

      # Return the result
      {
        success: true,
        id: result ? result.entityID : nil
      }
    end
    ...
    def create_pins(board, width, height, depth, angle, num_tails, offset_x, offset_y, offset_z)
      ...
      num_tails.times do |i|
        ...
        # Subtract the tail cutout from the pin area (use Group#subtract)
        pins_group = pins_group.subtract(tail_cutout_group)
        # subtract erases both inputs and returns new group
      end
    ...
    def create_board1_fingers(board, width, height, depth, num_fingers, offset_x, offset_y, offset_z)
      ...
      (num_fingers / 2).times do |i|
        ...
        # Subtract the cutout from the fingers (use Group#subtract)
        fingers_group = fingers_group.subtract(cutout_group)
        # subtract erases both inputs and returns new group
      end
    ...
    def create_board2_slots(board, width, height, depth, num_fingers, offset_x, offset_y, offset_z)
      ...
      (num_fingers / 2 + num_fingers % 2).times do |i|
        ...
        # Subtract the cutout from the board (use board group, not entities)
        board = board.subtract(cutout_group)
        # subtract erases both inputs and returns new group
      end
